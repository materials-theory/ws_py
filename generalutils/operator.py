...
class StrucOperators(object):
    ...
    def vectorlength(vec):
        ...
        Argument vector must be in the format of list with three components (e.g. ['0', '0', '0'] or [0, 0, 0])
        """
    ...
    def vectordistance(vec1, vec2):
        ...
        Each argument vector must be in the format of list with three components (e.g. ['0', '0', '0'] or [1, 1, 1])
        """
        v1 = None
        v2 = None
    ...
    def cross(vec1, vec2):
        ...
        cross_prod = [(float(vec1[1]) * float(vec2[2])) - (float(vec1[2]) * float(vec2[1])),
                      (float(vec1[2]) * float(vec2[0])) - (float(vec1[0]) * float(vec2[2])),
                      (float(vec1[0]) * float(vec2[1])) - (float(vec1[1]) * float(vec2[0]))]
    ...
    def volume(self, unitvec):
        """
        With a given set of unit vectors, calculates the volume of the cell
        If manually assigning the unit vector, it must follow the form of the list with three items:
        ex) [[0, 0, 0], [1, 1, 1], [2, 2, 2]] or
        ex2) [['0','0','0'], ['1','1','1'], ['2','2','2']]
        """

        bc_cross = self.cross(unitvec[1], unitvec[2])

        abc_product = [0, 0, 0]
        unit_cell_vol = 0.0

        for i in range(3):
            abc_product[i] = float(unitvec[0][i] * bc_cross[i])
            unit_cell_vol = float(abc_product[0] + abc_product[1] + abc_product[2])

        return unit_cell_vol

    def recvec(self, unitvec):
        """
        ...
        # Temporary list to store reciprocal vector information
        a_rec = [0, 0, 0]
        b_rec = [0, 0, 0]
        c_rec = [0, 0, 0]

        for i in range(3):
            a_rec[i] = float(self.cross(unitvec[1], unitvec[2])[i]) / self.volume(unitvec)
            b_rec[i] = float(self.cross(unitvec[2], unitvec[0])[i]) / self.volume(unitvec)
            c_rec[i] = float(self.cross(unitvec[0], unitvec[1])[i]) / self.volume(unitvec)

        return [a_rec, b_rec, c_rec]

    @staticmethod
    def directtocartesian(matrix, posdata):
        """
        Converts the position data from Direct coordinate to Cartesian coordinate.

        """
        return np.dot(posdata, matrix)

    @staticmethod
    def cartesiantodirect(matrix, posdata):
        """
        Converts the position data from Direct coordinate to Cartesian coordinate.

        """
        return np.dot(posdata, np.linalg.inv(matrix))
    ...
    def centroid(atom_pos, atom_weight=None):
        pos = np.array(atom_pos, dtype=np.float64)
    ...
    def angle(self, vec1, vec2):
        angle = math.degrees(math.acos(round(self.dot(vec1, vec2) /
                                             (self.vectorlength(vec1) * self.vectorlength(vec2)), 10)))
        return angle
